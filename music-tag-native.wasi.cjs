// napi-rs-artifact-metadata:{"version":2,"rootEntry":"index.js","exports":["MetaPicture","MusicFile"],"managedRootEntries":["browser.js","index.js","music-tag-native.wasm","music-tag-native.debug.wasm"]}
/* eslint-disable */
/* prettier-ignore */
...
let __wasmFilePath = __nodePath.join(__dirname, 'music-tag-native.wasm32-wasi.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'music-tag-native.wasm32-wasi.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('music-tag-native-wasm32-wasi')
  const __packagedWasmFilePath = __nodePath.join(
    __nodePath.dirname(__wasiPackageEntry),
    'music-tag-native.wasm32-wasi.wasm',
  )
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error(
      'music-tag-native-wasm32-wasi is installed but is missing music-tag-native.wasm32-wasi.wasm.',
    )
  }
...
module.exports.MusicFile = __napiModule.exports.MusicFile
